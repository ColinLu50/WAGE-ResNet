...
class NN(object):
	...
	def _ResNet20(self):
		...
		with tf.variable_scope('U4'):
			...
			with tf.variable_scope('C0'):
				x = self._activation(x)
				x = self._conv(x, 3, kernel_num, stride=2)
			with tf.variable_scope('C1'):
				x = self._activation(x)
				x = self._conv(x, 3, kernel_num)
				x = x + shortcut

		with tf.variable_scope('U5'):
			with tf.variable_scope('S0'):
				shortcut = tf.identity(x)
		...
		with tf.variable_scope('AVG'):
			x = self._activation(x)
			x = self._pool(x, 'AVG', ksize=8, stride=8)

		x = self._reshape(x)
		with tf.variable_scope('FC'):
			x = self._fc(x, self.shapeY[1], name='fc1')

		# for last layer(first layer in BP) error input quantization
	...
	def _batch_norm(self, x, data_format='NCHW'):
		x = batch_norm(x, center=True, scale=True, is_training=self.is_training, decay=0.9, epsilon=1e-5, fused=True,
